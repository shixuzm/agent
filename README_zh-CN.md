...
- **自动检查点**：当 token 使用超过阈值时，自动保存会话摘要。
- **上下文重建**：当接近上下文窗口上限时，从最新检查点、任务进展、相关记忆和近期消息重建精简上下文。
- **预算化注入**：检查点、任务进展、记忆按严格 token 预算注入，按相关性和优先级排序。
- **树状任务**：任务支持嵌套子任务（T1、T1.1、T1.2…），任务状态随检查点保留。
...
- **提供商与模型**：选择 LLM 提供商、模型、API Key 和 Base URL。
- **智能体**：管理内置智能体权限，并创建自定义智能体。
- **上下文**：配置上下文窗口、检查点阈值与预算比例。
- **记忆**：查看记忆统计并清空所有记忆（仅桌面端）。
- **MCP 服务器**：连接 Model Context Protocol 服务器以扩展工具能力。
- **快捷键与主题**：自定义快捷键，并在浅色/深色/跟随系统主题间切换。

### Compose 模式

使用 `/compose <spec>` 启动 spec 驱动的完整开发生命周期：

1. Plan — 理解 spec 并拆分为任务。
2. Execute — 调用技能/子智能体实现代码。
3. Review — 审查代码与结果。
4. TDD — 生成并运行测试。
5. Debug — 诊断并修复失败。
6. Validate — 运行检查清单、类型检查与构建验证。
7. Merge — 收尾并总结。

更多命令：

- `/dream` — 从近期会话中提取持久知识写入项目记忆。
- `/distill` — 发现重复性手工工作流，并建议可复用的技能/子智能体/命令。
